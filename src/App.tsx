...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f6f8] font-sans">
      {/* Header — 한 줄 구조: [제목] [단계 안내(정중앙)] [사용횟수 + Word 다운로드] */}
      <header className="bg-white/90 backdrop-blur border-b border-slate-200/80 sticky top-0 z-40 shadow-sm">
        <div className="w-full px-6 lg:px-10 py-4 lg:py-5 grid grid-cols-[1fr_auto_1fr] items-center gap-4">
          {/* 왼쪽: 서비스 제목 (큰 글씨) */}
          <a href="/teachers-meeting/" className="flex items-center gap-4 min-w-0 group justify-self-start" title="랜딩페이지로 이동">
            <span className="w-12 h-12 lg:w-14 lg:h-14 rounded-2xl bg-teal-500 flex items-center justify-center shadow-sm group-hover:bg-teal-600 transition-colors shrink-0">
              <NotebookPen className="w-7 h-7 lg:w-8 lg:h-8 text-white" />
            </span>
            <h1 className="font-extrabold text-2xl lg:text-[28px] text-teal-900 truncate group-hover:text-teal-600 transition-colors cursor-pointer tracking-tight">
              교사회의록 AI 작성도우미
            </h1>
          </a>

          {/* 가운데: 진행 단계 — grid 가운데 칸으로 화면 정중앙 배치 */}
          <nav className="hidden lg:flex items-center gap-2 justify-self-center" aria-label="진행 단계">
            {STEPS.map((step, i) => (
              <React.Fragment key={step}>
                <span
                  className={`px-3 py-1.5 rounded-full whitespace-nowrap text-[13px] font-medium ${
                    i === 0 || i === 1
          ...
          {/* 오른쪽: 사용횟수(작게) + Word 다운로드(크게) 한 줄 */}
          <div className="flex items-center gap-3 lg:gap-4 justify-self-end whitespace-nowrap">
            <span className={`hidden sm:inline-flex px-2.5 py-1 rounded-full text-xs font-semibold items-center gap-1.5 ${limitReached ? 'bg-red-50 text-red-500 border border-red-100' : 'bg-teal-50 text-teal-600 border border-teal-100'}`}>
            ...
            <button
              ...
              disabled={!hasResult}
              className={`flex items-center gap-2.5 px-6 lg:px-8 py-3.5 rounded-full text-base lg:text-lg font-bold transition-all whitespace-nowrap ${
                hasResult
